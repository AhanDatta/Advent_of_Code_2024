...
for i in range(len(test_values_arr)):
    ...
    Z3n_elements = list(itertools.product([0,1,2], repeat=len(code)-1)) #creates all elements of the set (Z/3Z)^n for n = len(code)-1

    for op_ordering in Z3n_elements:
        ...
        for j in range(len(code)-1):
            if op_ordering[j] == 0:
                ...
                operator_total *= code[j+1]
            elif op_ordering[j] == 2:
                operator_total = int(str(operator_total) + str(code[j+1]))
